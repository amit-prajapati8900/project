...
app.get("/home",validedata,asyncError(async (req, res, next) => {
  console.log("everything is ok");
  const alldata = await Data.find();
  if(!alldata){
   next(new ExpressError(400,"something went wruong "));
  }   
  res.render("show.ejs",{alldata,results: []});                   
}));
app.delete("/delete/:id",asyncError(async(req,res)=>{
   let {id} = req.params;
...
app.post("/newdata",validedata,asyncError(async(req,res)=>{
   let {name,deg,age,img} = req.body;
   const alldata = await Data({name,deg,age,img});
   alldata.save();
   res.redirect("/home");
}));
app.get("/update/:id",validedata,asyncError( async(req, res) => {
    let { id } = req.params;
    const record = await Data.findById(id);
    res.render("update.ejs", { data: record });
}));
app.patch("/update/:id",validedata,asyncError(async(req,res)=>{
   let {id} = req.params;
   let {name,deg,age,img} = req.body;
...
app.get('/search', async (req, res) => {
  ...
  try {
    const results = await User.find({ name: { $regex: searchQuery, $options: 'i' } });
    res.render('show', { alldata: [], results }); 
  } catch (err) {
    console.error(err);
