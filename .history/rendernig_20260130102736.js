...
app.get("/home",asyncError(async (req, res, next) => {
...
app.post("/newdata", asyncError(async(req,res)=>{
   let {name,deg,age,img} = req.body;
   const alldata = await Data({name,deg,age,img});
   alldata.save();
   res.redirect("/home");
}));
app.get("/update/:id",asyncError( async(req, res) => {
    let { id } = req.params;
    const record = await Data.findById(id);
    res.render("update.ejs", { data: record });
}));
app.patch("/update/:id",asyncError(async(req,res)=>{
   let {id} = req.params;
   let {name,deg,age,img} = req.body;
