...
const userSchma = require("userSchma");
...
const userSchma = require("./Error/userSchma.js");
...
app.get("/home",userSchma,asyncError(async (req, res, next) => {
...
app.delete("/delete/:id",userSchma,asyncError(async(req,res)=>{
   let {id} = req.params;
...
app.post("/newdata",userSchma,asyncError(async(req,res)=>{
   let {name,deg,age,img} = req.body;
   const alldata = await Data({name,deg,age,img});
   alldata.save();
   res.redirect("/home");
}));
app.get("/update/:id",userSchma,asyncError( async(req, res) => {
    let { id } = req.params;
    const record = await Data.findById(id);
    res.render("update.ejs", { data: record });
}));
app.patch("/update/:id",userSchma,asyncError(async(req,res)=>{
   let {id} = req.params;
   let {name,deg,age,img} = req.body;
