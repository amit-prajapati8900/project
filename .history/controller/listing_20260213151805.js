...
module.exports.patchdata = async (req, res) => {
  ...
  if (req.file) {
    ...
    // Preserve old image via hidden fields (nested names)
    img = {
      url: req.body["img[url]"],       // comes from <input name="img[url]">
      filename: req.body["img[filename]"] // comes from <input name="img[filename]">
    };
  }
