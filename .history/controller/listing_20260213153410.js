...
module.exports.patchdata = async (req, res) => {
  ...
  if (req.file) {
    // New file uploaded
    req.body.img = { url: req.file.path, filename: req.file.filename };
  } else {
    // Preserve old image via hidden fields
    req.body.img = {
      url: req.body["img[url]"],       // from <input name="img[url]">
      filename: req.body["img[filename]"] // from <input name="img[filename]">
    };
  }

  await Data.findByIdAndUpdate(
    id,
    { name, deg, age, img: req.body.img },
    { runValidators: true }
  );
