...
const Joi = require("./Error/joi.js");
...
app.get("/home",Joi,asyncError(async (req, res, next) => {
...
app.delete("/delete/:id",Joi,asyncError(async(req,res)=>{
   let {id} = req.params;
...
app.post("/newdata",Joi,asyncError(async(req,res)=>{
   let {name,deg,age,img} = req.body;
   const alldata = await Data({name,deg,age,img});
   alldata.save();
   res.redirect("/home");
}));
app.get("/update/:id",Joi,asyncError( async(req, res) => {
    let { id } = req.params;
    const record = await Data.findById(id);
    res.render("update.ejs", { data: record });
}));
app.patch("/update/:id",Joi,asyncError(async(req,res)=>{
   let {id} = req.params;
   let {name,deg,age,img} = req.body;
